...
export class InscripcionGeneralComponent implements OnInit, OnChanges {
  ...
  setPercentage_info(number, tab) {
    ...
    this.percentage_info = Math.round(this.utilidadesService.getSumArray(this.percentage_tab_info));
  ...
  setPercentage_acad(number, tab) {
    ...
    this.percentage_acad = Math.round(this.utilidadesService.getSumArray(this.percentage_tab_acad));
    this.setPercentage_total();
  }

  setPercentage_expe(number, tab) {
    this.percentage_tab_expe[tab] = (number * 100) / 1;
    this.percentage_expe = Math.round(this.utilidadesService.getSumArray(this.percentage_tab_expe));
    this.setPercentage_total();
  }

  setPercentage_proy(number, tab) {
    this.percentage_tab_proy[tab] = (number * 100) / 1;
    this.percentage_proy = Math.round(this.utilidadesService.getSumArray(this.percentage_tab_proy));
  }

  setPercentage_desc(number, tab) {
    this.percentage_tab_desc[tab] = (number * 100) / 1;
    this.percentage_desc = Math.round(this.utilidadesService.getSumArray(this.percentage_tab_desc));
  }

  setPercentage_docu(number, tab) {
    this.percentage_tab_docu[tab] = (number * 100) / 1;
    this.percentage_docu = Math.round(this.utilidadesService.getSumArray(this.percentage_tab_docu));
    this.setPercentage_total();
  }

  setPercentage_prod(number, tab) {
    this.percentage_tab_prod[tab] = (number * 100) / 1;
    this.percentage_prod = Math.round(this.utilidadesService.getSumArray(this.percentage_tab_prod));
    this.percentage_prod = (this.percentage_prod * 100) / this.percentage_prod;
  }

  setPercentage_total() {
    this.percentage_total = Math.round(this.utilidadesService.getSumArray(this.percentage_tab_info)) / 4;
    this.percentage_total += Math.round(this.utilidadesService.getSumArray(this.percentage_tab_acad)) / 4;
    this.percentage_total += Math.round(this.utilidadesService.getSumArray(this.percentage_tab_docu)) / 4;
    this.percentage_total += Math.round(this.utilidadesService.getSumArray(this.percentage_tab_expe)) / 4;
