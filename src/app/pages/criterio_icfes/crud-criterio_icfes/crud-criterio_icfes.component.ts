...
export class CrudCriterioIcfesComponent implements OnInit {
  ...
  public loadLists() {
    this.store.select((state) => state).subscribe(
      (list) => {
        this.formCriterioIcfes.campos[this.getIndexForm('Genero')].opciones = list.listGenero[0];
        this.formCriterioIcfes.campos[this.getIndexForm('EstadoCivil')].opciones = list.listEstadoCivil[0];
        this.formCriterioIcfes.campos[this.getIndexForm('TipoIdentificacion')].opciones = list.listTipoIdentificacion[0];
      },
    );
  }
