...
export class ListPeriodoComponent implements OnInit {
    ...
    this.settings = {
      ...
      add: {
        addButtonContent:
          ...
          this.translate.instant('periodo.tooltip_crear') +
          '"></i>',
        createButtonContent:
          '<i class="nb-checkmark" title="' +
          this.translate.instant('periodo.tooltip_guardar') +
          '"></i>',
        cancelButtonContent:
          '<i class="nb-close" title="' +
          this.translate.instant('periodo.tooltip_cancelar') +
          '"></i>',
      },
      edit: {
        editButtonContent:
          '<i class="nb-edit" title="' +
          this.translate.instant('periodo.tooltip_editar') +
          '"></i>',
        saveButtonContent:
          '<i class="nb-checkmark" title="' +
          this.translate.instant('periodo.tooltip_guargar') +
          '"></i>',
        cancelButtonContent:
          '<i class="nb-close" title="' +
          this.translate.instant('periodo.tooltip_cancelar') +
      ...
      delete: {
        deleteButtonContent:
          ...
          this.translate.instant('periodo.tooltip_eliminar') +
          '"></i>',
        confirmDelete: true,
      },
    };
  ...
  loadData(): void {
    this.parametrosService
      .get('periodo?query=CodigoAbreviacion:PA&limit=0')
      .subscribe(res => {
        if (res !== null) {
  ...
  loadAno() {
    this.parametrosService
      .get('periodo?query=CodigoAbreviacion:VG&fields=Year')
      .subscribe(
        res => {
          const r = <any>res;
    ...
    this.parametrosService
      .get(
        'periodo?query=Year:' +
          ...
          ',CodigoAbreviacion:PA&fields=Ciclo',
      )
      .subscribe(
        res => {
          const r = <any>res;
