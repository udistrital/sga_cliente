...
export class CrudIcfesComponent implements OnInit {
  ...
  constructor(
    ...
    this.formInfoFormacionAcademica = FORM_ICFES;
    this.construirForm();
  ...
  public loadLists() {
    this.store.select((state) => state).subscribe(
      (list) => {
       ...
       this.formInfoFormacionAcademica.campos[this.getIndexForm('numeroSemestres')].opciones = list.listSemestresSinEstudiar[0];
      },
   );
 }
